//! Training loop for the reset-rotor example. It builds the dataloaders, runs
//! the train and validation epochs, and saves the model and the optimizer. The
//! objective is a cross-entropy head over **every** position (the detent at
//! each step, [`forward_classification`]). A `Trainer` steps it. Under plain
...
use crate::dataset::{
    ...
    ResetRotorDataset, SEQ_LENGTH, TRAIN_SEED,
};
use burn::prelude::*;
use burn::{
...
/// The evaluation splits, reported separately. An ablated block fails on the two
/// adversarial families, so one averaged number would hide the result (see
/// [`crate::dataset`]).
...
/// Forward the model, and score the detent at **every** position.
