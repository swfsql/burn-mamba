//! Training loop for the reset-rotor example: builds the dataloaders, runs the
//! train/validate epochs, and checkpoints the model and optimizer. The objective
//! is a cross-entropy head over **every** position (the detent at each step,
//! [`forward_classification`]), stepped by a `Trainer`: under plain SGD (unless
//! `--no-graph`) the whole training step replays from a graph captured at the
//! first batch, under any other optimizer it steps eagerly.
...
/// where an ablated block fails, so a single averaged number would hide the
