...
cargo check --example mnist-ae-bidi --features "dev-ndarray"

# training and running inference in wgpu (fp32)
# note: the following requires ~7GB vram during training by default
cargo run --release --example mnist-ae-bidi --features "dev-wgpu,dev-autotune" -- --training --inference
```
...
# training and running inference in wgpu (fp32)
# note: the following requires ~7GB vram during training by default
cargo run --release --example mnist-ae-bidi --features "dev-cuda,dev-autotune" -- --training -a "/shared/burn-mamba/examples/mnist-ae-bidi/tmp/02"
```
