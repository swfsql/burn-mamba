...
/// The two-token chunk length this task passes to the shared
/// [`protocol::ssd_path`] (the default ≈32 chunk would zero-pad every
/// sequence 16×).
const CHUNK_LEN: Option<usize> = Some(2);

/// The SSD path used by chunkwise forwards (see [`protocol::ssd_path`]);
/// family follows the model (`--mamba3`), `chunk_len = 2`.
pub fn ssd_path(model: &MambaVocabNet) -> MambaSsdPath {
    protocol::ssd_path(model, CHUNK_LEN)
}

/// Final-position logits `[n, p]` (see [`protocol::final_logits`]).
pub fn final_logits(model: &MambaVocabNet, inputs_bs: &Tensor<2, Int>, stepwise: bool) -> Tensor<2> {
    protocol::final_logits(model, inputs_bs, stepwise, CHUNK_LEN)
}

/// [`final_logits`] with each layer's **attached** state moments (see
/// [`protocol::final_logits_with_moments`]).
pub fn final_logits_with_moments(
    model: &MambaVocabNet,
    inputs_bs: &Tensor<2, Int>,
) -> (Tensor<2>, Vec<StateMoments>) {
    protocol::final_logits_with_moments(model, inputs_bs, CHUNK_LEN)
}
...
/// Fraction of examples whose final-position argmax matches the label (see
/// [`protocol::accuracy`]).
pub fn accuracy(
    model: &MambaVocabNet,
    inputs_bs: &Tensor<2, Int>,
    ...
    protocol::accuracy(model, inputs_bs, labels_b, stepwise, CHUNK_LEN)
}
