...
pub fn combined_backward<B: Backend>(
    ...
    let (d_v_diag_bnlmhp, d_c_diag_bnlmhr, d_b_diag_bnlmhr, d_gamma_bnlh) = {
        ...
        let d_gamma_bnlh: Tensor<4> = (d_y_bnlhmp.clone() * y_d_unw_bnlhmp)
            .sum_dim(5) // bnlhm1
    ...
    let neg_inf_strict_ll: Tensor<2> =
        Tensor::full([chunk_len, chunk_len], f32::NEG_INFINITY, &device).triu(0);

    // ═══════════════════════════════════════════════════════════════════════
    // REVERSE PER-CHUNK LOOP — K5 (BLUE + LOWER) + K4 fused
    // ═══════════════════════════════════════════════════════════════════════
    let mut vec_lower_d_v_bhLMp: Vec<Tensor<4>> = Vec::with_capacity(nchunks);
    let mut vec_blue_d_c_bhLMr: Vec<Tensor<4>> = Vec::with_capacity(nchunks);
    let mut vec_d_cb_bhLMLM: Vec<Tensor<4>> = Vec::with_capacity(nchunks);
    let mut vec_blue_d_da_bhl: Vec<Tensor<3>> = Vec::with_capacity(nchunks);
    let mut vec_lower_d_da_bhl: Vec<Tensor<3>> = Vec::with_capacity(nchunks);
    let mut vec_lower_d_scale_bhl: Vec<Tensor<3>> = Vec::with_capacity(nchunks);
    ...
    for i_chunk in (0..nchunks).rev() {
        ...
        let cb_bhLMLM: Tensor<4> = cb_bnhLMLM
            .clone()
            .slice(s![.., i_chunk, .., .., ..])
            .squeeze_dim::<4>(1);

        let da_cumsum_bhLM: Tensor<3> = da_cumsum_bhnLM
            .clone()
            .slice(s![.., .., i_chunk, ..])
            .squeeze_dim::<3>(2);

        // scaleₜ per fused source position: scale[s_time] broadcast over s_m.
        let scale_bhLM: Tensor<3> = scale_bnlh
            .clone()
        ...
        let d_y_bhLMp: Tensor<4> = d_y_bnhLMp
            .clone()
            .slice(s![.., i_chunk, .., .., ..])
            .squeeze_dim::<4>(1);

        // ── BLUE backward (identical to double-ssd form) ─────────────────
        let exp_da_cumsum_bhLM: Tensor<3> = da_cumsum_bhLM.clone().exp();
        let exp_da_cumsum_bhLMp: Tensor<4> = exp_da_cumsum_bhLM
            .clone()
            .unsqueeze_dim::<4>(3)
            .expand([batch, nheads, chunk_len * mimo_rank, per_head_dim]);
        let d_ch_bhLMp: Tensor<4> = d_y_bhLMp.clone() * exp_da_cumsum_bhLMp.clone();
        san(&d_ch_bhLMp);

        let d_chunk_input_state_bhpr: Tensor<4> = c_bhLMr
            .clone()
        ...
        let d_c_blue_bhLMr: Tensor<4> =
            d_ch_bhLMp.clone().matmul(chunk_input_state_bhpr.clone());
        vec_blue_d_c_bhLMr.push(d_c_blue_bhLMr);

        let ch_bhLMp: Tensor<4> = c_bhLMr
            .clone()
            .matmul(chunk_input_state_bhpr.clone().permute([0, 1, 3, 2]));
        let d_da_blue_bhLM: Tensor<3> = (d_y_bhLMp.clone() * ch_bhLMp * exp_da_cumsum_bhLMp)
            .sum_dim(3)
            .squeeze_dim::<3>(3);
        let d_da_blue_bhl: Tensor<3> = d_da_blue_bhLM
            .reshape([batch, nheads, chunk_len, mimo_rank])
            .sum_dim(3)
            .squeeze_dim::<3>(3);
        ...
        let da_target_bhLMLM: Tensor<4> = da_cumsum_bhLM.clone().unsqueeze_dim::<4>(3).expand([
            batch,
            nheads,
            chunk_len * mimo_rank,
            chunk_len * mimo_rank,
        ]);
        let da_source_bhLMLM: Tensor<4> = da_cumsum_bhLM.unsqueeze_dim::<4>(2).expand([
            batch,
            nheads,
            chunk_len * mimo_rank,
            chunk_len * mimo_rank,
        ...
        let neg_inf_mimo_bhLMLM: Tensor<4> = neg_inf_strict_ll
            .clone()
        ...
        let scale_col_bhLMLM: Tensor<4> = scale_bhLM
        ...
        // d_w = d_y · vᵀ
        let d_w_bhLMLM: Tensor<4> = d_y_bhLMp
            .clone()
            .matmul(v_bhLMp.clone().permute([0, 1, 3, 2]));
        san(&d_w_bhLMLM);

        // d_v_lower = wᵀ · d_y
        let d_v_lower_bhLMp: Tensor<4> =
        ...
        let d_da_target_bhLM: Tensor<3> = d_diff_bhLMLM.clone().sum_dim(3).squeeze_dim::<3>(3);
        let d_da_source_bhLM: Tensor<3> = d_diff_bhLMLM.sum_dim(2).squeeze_dim::<3>(2);
        let d_da_lower_bhLM = d_da_target_bhLM - d_da_source_bhLM;
        let d_da_lower_bhl: Tensor<3> = d_da_lower_bhLM
            .reshape([batch, nheads, chunk_len, mimo_rank])
            .sum_dim(3)
            .squeeze_dim::<3>(3);
        ...
        let d_scale_lower_bhl: Tensor<3> = d_scale_at_bhLMLM
    ...
    let d_v_lower_bnhLMp: Tensor<5> = Tensor::stack(vec_lower_d_v_bhLMp, 1);
    let d_c_blue_bnhLMr: Tensor<5> = Tensor::stack(vec_blue_d_c_bhLMr, 1);
    let d_cb_bnhLMLM: Tensor<5> = Tensor::stack(vec_d_cb_bhLMLM, 1);
    let d_da_blue_bhnl: Tensor<4> = Tensor::stack(vec_blue_d_da_bhl, 2);
    let d_da_lower_bhnl: Tensor<4> = Tensor::stack(vec_lower_d_da_bhl, 2);
    let d_scale_lower_bhnl: Tensor<4> = Tensor::stack(vec_lower_d_scale_bhl, 2);
    ...
    let d_da_cumsum_k4_bhnl: Tensor<4> = {
    ...
    let d_decayed_v_bnhpLM: Tensor<5> = d_intra_chunk_state_bnhpr
        .clone()
        .matmul(k_scaled_bnhLMr.clone().permute([0, 1, 2, 4, 3])); // k_scaled_bnhrLM
    let d_k_scaled_bnhLMr: Tensor<5> = decayed_v_bnhpLM
    ...
    let d_decay_bhnLM: Tensor<4> = (d_decayed_v_bnLMhp.clone() * v_bnLMhp)
        .sum_dim(4)
        .squeeze_dim::<4>(4)
        .permute([0, 3, 1, 2]);
    ...
    let k3_decay_bnLMh1 = k3_decay_bhnLM
        ...
        .permute([0, 2, 3, 1])
        .unsqueeze_dim::<5>(4);
    let d_v_k3_bnLMhp: Tensor<5> = d_decayed_v_bnLMhp * k3_decay_bnLMh1;
    let d_v_k3_bnlmhp: Tensor<6> =
    ...
    let d_a_cumsum_last_bhn: Tensor<3> = d_decay_times_decay_bhnLM
        .clone()
        .sum_dim(3)
        .squeeze_dim::<3>(3);
    let d_da_cumsum_bhnLM = -d_decay_times_decay_bhnLM;

    let d_da_cumsum_k3_from_fused_bhnl: Tensor<4> = d_da_cumsum_bhnLM
        .reshape([batch, nheads, nchunks, chunk_len, mimo_rank])
        .sum_dim(4)
        .squeeze_dim::<4>(4);
    let d_da_cumsum_k3_from_last_bhnl: Tensor<4> = {
        let zeros = Tensor::<4>::zeros([batch, nheads, nchunks, chunk_len - 1], &device);
    ...
    let d_k_scaled_bnlmhr: Tensor<6> = d_k_scaled_bnhLMr
        .permute([0, 1, 3, 2, 4]) // bnLMhr
        .reshape([batch, nchunks, chunk_len, mimo_rank, nheads, state_rank]);
    let d_b_k3_bnlmhr: Tensor<6> = d_k_scaled_bnlmhr.clone() * scale_bnlh11;
    let d_scale_k3_bnlh: Tensor<4> = (d_k_scaled_bnlmhr * b_bnlmhr.clone())
    ...
    let d_c_k2_bnhLMr: Tensor<5> = d_cb_bnhLMLM.clone().matmul(b_for_k2_bnhLMr);
    let d_b_k2_bnhrLM: Tensor<5> = c_bnhLMr.permute([0, 1, 2, 4, 3]).matmul(d_cb_bnhLMLM);

    let d_c_k2_bnlmhr: Tensor<6> = d_c_k2_bnhLMr
        .permute([0, 1, 3, 2, 4])
        .reshape([batch, nchunks, chunk_len, mimo_rank, nheads, state_rank]);
    let d_b_k2_bnlmhr: Tensor<6> = d_b_k2_bnhrLM
        .permute([0, 1, 4, 2, 3])
        .reshape([batch, nchunks, chunk_len, mimo_rank, nheads, state_rank]);

    // ── Unstack d_c_blue / d_v_lower and reshape back ─────────────────────
    let d_c_blue_bnlmhr: Tensor<6> = d_c_blue_bnhLMr
        .permute([0, 1, 3, 2, 4])
        .reshape([batch, nchunks, chunk_len, mimo_rank, nheads, state_rank]);
    let d_v_lower_bnlmhp: Tensor<6> = d_v_lower_bnhLMp.permute([0, 1, 3, 2, 4]).reshape([
        batch,
        nchunks,
        chunk_len,
        mimo_rank,
