...
use crate::mamba3::ssd::trap_serial_recalculated::{
    Mamba3TrapBackendExt,
    combined_backward::{self, CombinedTrapGrads},
};
...
impl<B: Backend + Mamba3TrapBackendExt, C: CheckpointStrategy> Mamba3TrapBackendExt
    for Autodiff<B, C>
{
    /// Memory-efficient combined forward+backward for the Mamba-3 MIMO
    /// merged-form (trapezoidal) SSD.
    ///
    ...
    fn ssd_trap_serial_recalculated(
        ...
        impl<B: Backend + Mamba3TrapBackendExt> Backward<B, 7> for CombinedKernelsBackward {
            type State = State<B>;
            ...
            .stateful()
        {
            OpsKind::Tracked(prep) => {
                let (prim_y_bnlmhp, prim_final_state_bhpr) = B::ssd_trap_serial_recalculated(
